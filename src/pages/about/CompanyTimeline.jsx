...
const CompanyTimeline = () => {
  ...
  const timelineDataRaw = t("aboutUs.companyTimeline.items", {
    returnObjects: true,
  });

  const timelineData = Array.isArray(timelineDataRaw) ? timelineDataRaw : [];

  const title = t("aboutUs.companyTimeline.title");
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (lineFillRef.current && lineRef.current) {
        gsap.fromTo(
          lineFillRef.current,
          {
            scaleY: 0,
      ...
      dotRefs.current.forEach((dot, i) => {
        if (!dot || !yearRefs.current[i] || !cardRefs.current[i]) return;
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes dotPulse {
          0%,100% {
            box-shadow: 0 0 0 4px rgba(255,255,255,0.15), 0 0 0 8px rgba(255,255,255,0.05);
          }
          50% {
            box-shadow: 0 0 0 7px rgba(255,255,255,0.22), 0 0 0 14px rgba(255,255,255,0.06);
          }
        }
      ...
      <section
        ref={sectionRef}
        className="tl-wrap relative w-full overflow-hidden bg-gradient-to-r from-[#0C1A0E] via-[#2f7f35] to-[#0C1A0E] px-4 py-28 sm:px-8 lg:px-16"
      >
        {/* Glows */}
        ...
        <div className="relative z-10 mb-20 text-center">
          ...
          >
            ...
            <span className="text-[11px] font-bold uppercase tracking-[0.22em] text-white/65">
              {t("aboutUs.companyTimeline.badge")}
            </span>
          </div>
          ...
          <p className="tl-sub mx-auto max-w-md text-[15px] font-medium leading-relaxed text-white/45">
            {t("aboutUs.companyTimeline.subtitle")}
          </p>
        </div>
        ...
        <div className="relative z-10 mx-auto max-w-5xl">
          ...
          <div
            ref={lineRef}
            className="absolute left-1/2 top-0 h-full w-[2px] -translate-x-1/2 md:left-1/2 max-md:left-5"
            style={{
            ...
            {timelineData.map((item, i) => {
              ...
              const Card = (
                ...
                >
                  ...
                  <div className="tl-mobile-year mb-3 hidden rounded-lg border border-white/10 bg-white/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-white/70">
                    {t("aboutUs.companyTimeline.yearLabel")} {item.year}
                  </div>
              ...
              const Year = (
                ...
                >
                  <span className="mb-0.5 text-[9px] font-black uppercase tracking-[0.22em] text-white/30">
                    {t("aboutUs.companyTimeline.yearLabel")}
                  </span>
              ...
              return (
                <div key={`${item.year}-${i}`} className="tl-row">
                  {/* Left column */}
          ...
          <div className="mt-16 flex flex-col items-center gap-3">
            ...
            <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-white/28">
              {t("aboutUs.companyTimeline.endText")}
            </p>
          </div>
        </div>
      </section>
