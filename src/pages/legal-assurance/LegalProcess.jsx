...
const LegalProcess = () => {
  ...
  const leftItems = t("employers.legalAssurancePage.legalProcess.leftItems", {
    returnObjects: true,
  });
  const processSteps = t(
    "employers.legalAssurancePage.legalProcess.processSteps",
    { returnObjects: true },
  );

  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      gsap.set(leftRef.current, { x: -45, opacity: 0, filter: "blur(8px)" });
      gsap.set(itemsRef.current, { x: 50, opacity: 0, filter: "blur(8px)" });
      ...
      <div className="relative z-20 mx-auto grid min-h-[360px] max-w-7xl grid-cols-1 items-center gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[0.72fr_1.28fr] lg:px-16">
        <div ref={leftRef} className="lg:pl-14">
          <p className="mb-3 text-[13px] font-black text-[#ff2d18]">
            {t("employers.legalAssurancePage.legalProcess.badge")}
          </p>
          <h2 className="max-w-[320px] text-[36px] font-black leading-[0.95] text-white drop-shadow-[0_8px_18px_rgba(0,0,0,0.55)] sm:text-[48px]">
            {t("employers.legalAssurancePage.legalProcess.heading")}
          </h2>
          <ul className="mt-5 space-y-2">
            {leftItems.map((item, idx) => (
              ...
              >
                <span className="text-[#ff2d18]">»</span> {item}
              </li>
            ))}
          </ul>
        </div>
